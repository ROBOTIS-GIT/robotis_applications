...
class VRTrajectoryPublisher(Node):
    ...
    def __init__(self):
        ...
        self.wrist_reference_offset = np.array([
            self.get_parameter(
                'wrist_reference_forward_offset_m'
            ).get_parameter_value().double_value,
            0.0,
            0.0,
        ], dtype=np.float64)
        ...
        # Wrist pose publishers for visualization and control
        ...
        self.previous_camera_body_matrix = None
        self.accumulated_camera_yaw = 0.0
        self.current_odom = None
        self.initial_odom_position = None
        self.initial_odom_yaw = None
        self.previous_odom_yaw = None
        self.accumulated_odom_yaw = 0.0
    ...
    def reactivate_callback(self, msg):
        ...
        if new_state:
            ...
            self.teleop_head_transform_matrix = self.head_transform_matrix.copy()
            self.teleop_torso_transform_matrix = (
                self.torso_transform_matrix.copy()
                if self.torso_transform_matrix is not None else None
            )
            self.calibration_input = {'left': {}, 'right': {}}
            self.calibration_anchor = {'left': {}, 'right': {}}
        else:
    ...
    def _arm_tracking_head_matrix(self):
        ...
        Base planar translation/yaw must always stay out of a base_link-relative
        wrist target: /cmd_vel owns that motion, and injecting it into the wrist
        makes the arms absorb base tracking lag. Lift is different because it is
        an internal joint under base_link, so enabled lift displacement remains
        in the arm target on Z.
        """
        current_head = self.head_transform_matrix
        initial_head = self.teleop_head_transform_matrix
        current = (
            self.torso_transform_matrix
            if self.torso_transform_matrix is not None else current_head
        )
        initial = (
            self.teleop_torso_transform_matrix
            if self.teleop_torso_transform_matrix is not None else initial_head
        )
        if not self.is_vr_publishing_active() or initial is None:
            return current

        current_pos_ros = self.vr_to_ros_matrix @ current[:3, 3]
        initial_pos_ros = self.vr_to_ros_matrix @ initial[:3, 3]
        tracking_pos_ros = current_pos_ros.copy()
        if self.enable_lift_publishing:
            tracking_pos_ros[2] = initial_pos_ros[2]

        current_rot_ros = R.from_matrix(
            self.vr_to_ros_matrix @ current[:3, :3]
        )
        tracking_rot_ros = current_rot_ros

        tracking = np.eye(4, dtype=np.float64)
        tracking[:3, 3] = self.vr_to_ros_matrix.T @ tracking_pos_ros
        tracking[:3, :3] = self.vr_to_ros_matrix.T @ tracking_rot_ros.as_matrix()
        return tracking

    def _guide_tracking_matrix(self):
        """Return a visualization frame that never changes on teleop toggles.

        Control calibration is intentionally reset on every enable, but the VR
        guide must not switch between current and newly anchored torso frames.
        Keep the lift-owned Z axis anchored for the lifetime of the Vuer session;
        planar pose follows the current torso because robot wrist TF is expressed
        in base_link, not odom.
        """
        current = (
            self.torso_transform_matrix
            if self.torso_transform_matrix is not None
            else self.head_transform_matrix
        )
        initial = (
            self.guide_torso_anchor_matrix
            if self.guide_torso_anchor_matrix is not None
            else self.guide_head_anchor_matrix
        )
        if initial is None:
            return current
    ...
    def _update_robot_wrist_poses_from_tf(self):
        ...
        for side, child_frame in self.wrist_tf_frames.items():
            try:
                ...
                if now_sec - self.guide_tf_warn_time[side] >= 5.0:
                    self.get_logger().warn(
                        f'[VR CALIBRATION] Waiting for TF '
                        f'{self.guide_base_frame} -> {child_frame}: {exc}'
                    )
            ...
            if not self.robot_wrist_pose_logged[side]:
                self.get_logger().info(
                    f'[VR CALIBRATION] wrist TF {self.guide_base_frame} -> {child_frame}: '
                    f'[{p.x:+.3f}, {p.y:+.3f}, {p.z:+.3f}]'
                )
                self.robot_wrist_pose_logged[side] = True

    def _apply_relative_goal(self, side, role, position, rotation):
        """Anchor tracked motion to the measured follower pose at calibration."""
        if role == 'wrist' and self.calibration_pending:
            robot_pose = self.robot_wrist_pose[side]
            if robot_pose is not None:
                self.calibration_input[side][role] = (position.copy(), rotation)
                robot_position, robot_rotation = robot_pose
                # The forward offset is added to every published wrist goal below.
                # Compensate it in the anchor so the first goal after calibration
                # remains exactly at the measured robot pose (no enable-time step).
                self.calibration_anchor[side][role] = (
                    robot_position - self.wrist_reference_offset,
                    robot_rotation,
                )
                if all('wrist' in self.calibration_input[s] for s in ('left', 'right')):
                    self.calibration_pending = False
                    self.relative_calibrated = True
                    self._publish_reactivate_state(True)
                    self.get_logger().info(
                        '[CALIBRATION] Both wrist references captured; '
                        'relative teleoperation enabled.'
                    )
        if not self.relative_calibrated:
            return None
        if role not in self.calibration_input[side]:
            self.calibration_input[side][role] = (position.copy(), rotation)
            wrist_input, _ = self.calibration_input[side]['wrist']
            wrist_anchor, wrist_anchor_rot = self.calibration_anchor[side]['wrist']
            self.calibration_anchor[side][role] = (
                wrist_anchor + position - wrist_input, wrist_anchor_rot
            )
    ...
    def body_yaw_delta(self, from_matrix, to_matrix):
        """Return torso yaw change without assuming a skeleton forward axis."""
        if from_matrix is None or to_matrix is None:
            return 0.0
        relative_vr = (
            to_matrix[:3, :3] @ from_matrix[:3, :3].T
        )
        relative_ros = (
            self.vr_to_ros_matrix @ relative_vr @ self.vr_to_ros_matrix.T
        )
        forward_ros = relative_ros @ np.array(
            [1.0, 0.0, 0.0], dtype=np.float64
        )
        fx, fy = float(forward_ros[0]), float(forward_ros[1])
    ...
    def publish_relative_pose(
        ...
        # With lift enabled, _arm_tracking_head_matrix() freezes the tracking
        # origin on Z. The torso height delta is therefore already included in
        # camera_relative_position and gets multiplied by vr_scale above. Body
        # motion must follow the lift 1:1; only hand motion should use the VR
        # gain. Remove the excess (vr_scale - 1) contribution instead of adding
        # the height delta a second time.
        if (
            self.enable_lift_publishing
            and self.apply_head_height_to_arm_z
            and pose_role in ('wrist', 'shoulder')
        ):
        ...
        relative_goal = self._apply_relative_goal(
            ...
            base_position, R.from_quat(arm_quaternion)
        )
        if relative_goal is None:
            return
        base_position, relative_rotation = relative_goal
        if pose_role == 'wrist':
            # Apply the operator-to-robot alignment offset after relative
            # calibration so it is not canceled while capturing the anchor.
            base_position = base_position + self.wrist_reference_offset
        arm_quaternion = relative_rotation.as_quat()
        ...
        target_pose.pose.position.x = base_position[0]
    ...
    def process_hand_joints(self, hand_data, side='left', pose_stamp=None):
        ...
        head_rot_inv = (
            tracking_head[:3, :3].T if self.hand_pose_is_head_relative else None
        )
        head_world_pos = (
            tracking_head[:3, 3] if self.hand_pose_is_head_relative else None
        )
    ...
    def _base_wrist_pose_to_vr_world(self, side, cached):
        ...
        # on the tracked human wrist instead of falsifying the robot pose.
        guide_base_pos = base_pos
        # wrist_vr_scale is a teleoperation motion gain, not a physical frame
        # scale. Applying its inverse here made an extended robot arm appear
        # artificially shortened in VR.
        ros_pos = (
            guide_base_pos
            - np.array([offsets['x'], offsets['y'], offsets['z']])
            + self.zedm_to_base_offset
        )
        relative = np.eye(4)
        relative[:3, :3] = (
    ...
    def _robot_wrist_to_vr_world(self, side):
        ...
        if not self.guide_transform_logged[side]:
            self.get_logger().info(
                ...
                f'{relative[:3, 3].round(3).tolist()}, '
                f'world={world[:3, 3].round(3).tolist()}'
            )
            self.guide_transform_logged[side] = True
        return world

    def _calibration_hand_guide(self, side, matrix):
        """Create a lightweight translucent wrist sphere with a small frame."""
        position = matrix[:3, 3].astype(float).tolist()
        quaternion = R.from_matrix(matrix[:3, :3]).as_quat().tolist()
        return Group(
            key=f'{side}-hand-calibration-guide',
            position=position,
            quaternion=quaternion,
            children=[
                Sphere(
                    key=f'{side}-wrist-sphere',
                    args=[0.045, 16, 12],
                    # Follower robot wrist: cyan-blue.
                    materialType='basic',
                    material={
                        'color': '#00b7ff',
                        'transparent': True,
                ...
                CoordsMarker(
                    key=f'{side}-wrist-frame', scale=0.07,
                    opacity=1.0, depthTest=False, renderOrder=10,
                ),
            ],
        )
    ...
    def _operator_wrist_candidate_from_tracking(self, side):
        ...
        if hand_data is None or len(hand_data) != 400:
            return None
        try:
            wrist = np.asarray(hand_data[:16], dtype=np.float64).reshape(
                4, 4, order='F'
            )
            world_rot = wrist[:3, :3]
    ...
    def _tracked_wrist_guide(self, side, matrix):
        ...
        quaternion = R.from_matrix(matrix[:3, :3]).as_quat().tolist()
        return Group(
            key=f'{side}-tracked-wrist-guide',
            position=position,
            quaternion=quaternion,
            children=[
                Sphere(
                ...
                CoordsMarker(
                    key=f'{side}-tracked-wrist-frame', scale=0.07,
                    opacity=1.0, depthTest=False, renderOrder=12,
                ),
            ],
        )
            ...
            while True:
                ...
                for side in ('left', 'right'):
                    ...
                    if matrix is not None:
                        guides.append(self._calibration_hand_guide(side, matrix))
                    tracked_matrix = self._tracked_wrist_to_vr_world(side)
                    if tracked_matrix is not None:
    ...
    def _update_gesture_toggle(self, left_state, right_state):
        ...
        if new_state:
            ...
            self.calibration_anchor = {'left': {}, 'right': {}}
            self.get_logger().info(
                '[GESTURE] Calibration accepted; capturing both wrist poses before activation.'
            )
        else:
            self.relative_calibrated = False
            self.calibration_pending = False
            ...
            self.teleop_torso_transform_matrix = None
            self._publish_reactivate_state(False)
            self.get_logger().info('[GESTURE] VR teleop disabled.')
    ...
    async def on_body_tracking_move(self, event, session):
        """
        Handle body tracking events (head, lift, base, shoulder poses).
        ...
        try:
            ...
            if head_matrix is not None:
                ...
                # Keep the headset transform live during calibration so the measured
                # follower wrist guides stay fixed at the correct world location.
                ...
                current_camera_height = float(body_ros_pos[2])
                current_camera_position = np.array(
                    [body_ros_pos[0], body_ros_pos[1]], dtype=np.float64
                )
                r = R.from_quat(ros_quat)
                ...
                if self.initial_camera_height is None:
                    ...
                    self.initial_camera_yaw = 0.0
                    self.previous_camera_position = current_camera_position.copy()
                    self.previous_camera_yaw = 0.0
                    self.previous_camera_body_matrix = body_motion_matrix.copy()
                    self.accumulated_camera_yaw = 0.0
                ...
                if self.previous_camera_body_matrix is not None:
                    self.accumulated_camera_yaw += self.body_yaw_delta(
                        self.previous_camera_body_matrix, body_motion_matrix
                    )
                self.previous_camera_body_matrix = body_motion_matrix.copy()
                self.previous_camera_yaw = self.accumulated_camera_yaw
                relative_yaw = self.accumulated_camera_yaw
                ...
                # Shared arm-pose timestamp keeps left/right wrist/shoulder aligned.
                ...
                if (self.initial_camera_position is not None
                    ...
                    if self.previous_odom_yaw is not None:
                        self.accumulated_odom_yaw += self.wrap_pi(
                            current_odom_yaw - self.previous_odom_yaw
                        )
                    self.previous_odom_yaw = current_odom_yaw
                    robot_movement_yaw = self.accumulated_odom_yaw
    ...
    def publish_body_joint_pose(
        self, joint_matrix, publisher, side='', pose_role='shoulder', stamp=None
    ):
        """Publish PoseStamped for a tracked body joint."""
        tracking_head_inverse = np.linalg.inv(self._arm_tracking_head_matrix())
        relative_joint_matrix = tracking_head_inverse @ joint_matrix
